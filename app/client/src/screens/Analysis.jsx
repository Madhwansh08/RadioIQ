...
const Analysis = () => {
          ...
          drawImageRef.current((ctx, { scaleToFit, offsetX, offsetY }) => {
            if (segmentationActive) {
              abnormalities.forEach(abn => {
                if (!abn.segmentation || !Array.isArray(abn.segmentation) || abn.segmentation.length === 0) {
                  return;
                }
                ctx.save();
                ...
                ctx.closePath();
                ctx.setLineDash([5, 5]);
                ctx.strokeStyle = abnormalitiesMap[abn.name]?.color || "magenta";
                ctx.lineWidth = 2 / (scale * scaleToFit);
                ctx.stroke();
                ctx.globalAlpha = 0.05;
                ctx.fillStyle = abnormalitiesMap[abn.name]?.color || "magenta";
                ctx.fill();
                ctx.restore();
              });
            }
  ...
  const renderFilterSlider = filter => (
    <div className="flex flex-col bg-gray-900 p-4 shadow-lg rounded-md w-full max-w-xs sm:w-64 mx-4 sm:mx-0 z-50">
      ...
      <button
        ...
        className="mt-4 p-2 bg-indigo-600 text-white font-semibold rounded hover:bg-gray-800 border hover:border-indigo-600 transition-colors"
      >
        Close
      </button>
    </div>
  ...
  const MobileToolbar = () => {
    ...
    return (
      <motion.div
        onMouseMove={(e) => mouseY.set(e.clientY)}
        onMouseLeave={() => mouseY.set(Infinity)}
        className="flex flex-row items-center gap-x-4 p-2 bg-[#030811] fixed bottom-0 left-0 w-full z-10"
      >
        <CustomTooltip title="AI Annotation">
          <ToolbarButton
            onClick={handleSegmentationToggle}
        ...
        <CustomTooltip title="More Tools">
          <ToolbarButton
            onClick={() => setIsToolbarDrawerOpen((prev) => !prev)}
            className="dark:bg-[#030811] bg-[#fdfdfd] dark:text-[#fdfdfd] text-[#030811] dark:hover:bg-[#5c60c6] hover:bg-[#5c60c6]"
            mouseY={mouseY}
            label="More Tools"
          >
  ...
  return (
    <div className="flex md:flex-row flex-col min-h-screen dark:bg-gradient-to-b dark:bg-[#030811] bg-[#fdfdfd]">
      ...
      <div className="md:hidden fixed top-0 left-0 w-full bg-[#030811] p-2 flex justify-between items-center z-10">
        <button onClick={() => setIsPatientDrawerOpen(true)} className="mr-2 text-[#fdfdfd]">
          <FaUser size={20} />
        </button>
        ...
        <button onClick={() => setIsAnalysisDrawerOpen(true)} className="text-[#fdfdfd]">
          <BsThreeDots size={20} />
        </button>
      </div>
        ...
        <div className="text-center items-center">
          ...
          <div className="text-center items-center">
            <p className="text-sm font-medium italic">
              Note: This is an AI generated diagnosis. The findings are to be used for diagnostic purposes in consultation with a licensed medical expert.
            </p>
          </div>
        </div>
      </div>
