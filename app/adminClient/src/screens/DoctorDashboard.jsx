...
export default function DoctorDashboard({ doctors, fetchDoctors, setDoctors }) {
  ...
  const verifyDoctor = async (id) => {
    try {
      await axios.patch(`${config.API_URL}/admin/doctors/${id}/verify`, {}, { withCredentials: true });
  ...
  const blockDoctor = async (id) => {
    try {
      await axios.patch(`${config.API_URL}/admin/doctors/${id}/block`, {}, { withCredentials: true });
  ...
  const unblockDoctor = async (id) => {
    try {
      await axios.patch(`${config.API_URL}/admin/doctors/${id}/unblock`, {}, { withCredentials: true });
  ...
  const deleteDoctor = async (id) => {
    try {
      await axios.delete(`${config.API_URL}/admin/doctors/${id}`, { withCredentials: true });
            ...
            {doctors.map((doc) => (
              <tr key={doc._id}>
                ...
                <td className="border px-4 py-2">
                  {doc.isVerified ? (
                    <span className="bg-green-100 text-green-800 px-2 py-1 text-sm rounded-full">Verified</span>
                  ) : (
                    <button
                ...
                <td className="border px-4 py-2">
                  {doc.isBlocked ? (
                    <span className="bg-red-100 text-red-800 px-2 py-1 text-sm rounded-full">Blocked</span>
                  ) : (
                    <span className="bg-blue-100 text-blue-800 px-2 py-1 text-sm rounded-full">Active</span>
                  )}
                </td>
                <td className="border px-4 py-2">
                  <button
                    className="bg-yellow-500 text-white px-2 py-1 rounded"
                    onClick={() => doc.isBlocked ? unblockDoctor(doc._id) : blockDoctor(doc._id)}
                  >
